...
- **RetailOS Import API authentication and idempotency must be reviewed before implementation.** Owner/action: engineering/security must define inbound auth, tenant scoping, replay handling, idempotency keys, rate limits, and audit evidence before exposing ingestion endpoints.
...
- Integration Hub UI/data-source setup flow is implemented on branch `phase-0-5-integration-hub-ui` and pending review.
