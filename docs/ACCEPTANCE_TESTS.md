...
- Phase 1 inventory core foundation evidence: `tests/integration/phase1-inventory-core.test.ts` verifies stock adjustment approval writes movement ledger rows, transfer approval writes paired transfer ledger rows, stock counts create variance reconciliation issues, inventory search works by SKU/barcode within effective location scope, and cross-tenant/under-privileged inventory operations are denied.
